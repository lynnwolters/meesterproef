<?php

// adding the CSS and JS files

function bofesto_setup(){
    wp_enqueue_style('google-font', '//fonts.googleapis.com/css2?family=Edu+SA+Beginner:wght@400;500;600;700&display=swap');
    wp_enqueue_style('style', get_stylesheet_uri(), NULL, microtime(), 'all');

	// wp_enqueue_style( 'carrousel', get_template_directory_uri() . '/styles/carrousel.css');
	wp_enqueue_style( 'carrousel', get_template_directory_uri() . '/styles/carrousel-update.css');
	wp_enqueue_style( 'overons', get_template_directory_uri() . '/styles/over-ons.css');
	wp_enqueue_style( 'contact', get_template_directory_uri() . '/styles/contact.css');
	wp_enqueue_style( 'menu', get_template_directory_uri() . '/styles/menu.css');
	wp_enqueue_style( 'loader', get_template_directory_uri() . '/styles/loader.css');
	wp_enqueue_style( 'transitions', get_template_directory_uri() . '/styles/transitions.css');
	wp_enqueue_style( 'popup', get_template_directory_uri() . '/styles/pop-up.css');
	wp_enqueue_style( 'popup-propshop', get_template_directory_uri() . '/styles/pop-up-propshop.css');
	wp_enqueue_style( 'prop-room', get_template_directory_uri() . '/styles/prop-room.css');
	wp_enqueue_style( 'custom-cursor', get_template_directory_uri() . '/styles/custom-cursor.css');
	wp_enqueue_style( 'single', get_template_directory_uri() . '/styles/single.css');

    wp_enqueue_script("main", get_theme_file_uri('/js/script.js'), NULL, microtime(), true);
	wp_enqueue_script("pop-up", get_theme_file_uri('/js/pop-up.js'), NULL, microtime(), true);
	wp_enqueue_script("pop-up-propshop", get_theme_file_uri('/js/pop-up-propshop.js'), NULL, microtime(), true);
}

add_action('wp_enqueue_scripts', 'bofesto_setup');

add_filter( 'show_admin_bar', '__return_false');

// Adding theme Support

function bofesto_init() {
    add_theme_support('post-thumbnails');
    add_theme_support('title-tag');
    add_theme_support('html5',
        array('comment-list', 'comment-form', 'search-form')
    );
}

add_action('after_setup_theme', 'bofesto_init');

function register_my_menus() {
    register_nav_menus(
      array(
        'header-menu' => __( 'Header Menu' ),
        'extra-menu' => __( 'Extra Menu' )
       )
     );
   }
add_action( 'init', 'register_my_menus' );

//projects post type


// ACF
add_action( 'acf/include_fields', function() {
	if ( ! function_exists( 'acf_add_local_field_group' ) ) {
		return;
	}

	acf_add_local_field_group( array(
	'key' => 'group_6656eae2e329b',
	'title' => 'Portfolio',
	'fields' => array(
		array(
			'key' => 'field_6656eae3bd7d6',
			'label' => 'Image',
			'name' => 'image',
			'aria-label' => '',
			'type' => 'image',
			'instructions' => '',
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'return_format' => 'array',
			'library' => 'all',
			'min_width' => '',
			'min_height' => '',
			'min_size' => '',
			'max_width' => '',
			'max_height' => '',
			'max_size' => '',
			'mime_types' => '',
			'preview_size' => 'medium',
		),
		array(
			'key' => 'field_6666eae8adb65',
			'label' => 'Image_2',
			'name' => 'Image_2',
			'aria-label' => '',
			'type' => 'image',
			'instructions' => '',
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'return_format' => 'array',
			'library' => 'all',
			'min_width' => '',
			'min_height' => '',
			'min_size' => '',
			'max_width' => '',
			'max_height' => '',
			'max_size' => '',
			'mime_types' => '',
			'preview_size' => 'medium',
		),
		array(
			'key' => 'field_6666eb01adb66',
			'label' => 'Image_3',
			'name' => 'image_3',
			'aria-label' => '',
			'type' => 'image',
			'instructions' => '',
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'return_format' => 'array',
			'library' => 'all',
			'min_width' => '',
			'min_height' => '',
			'min_size' => '',
			'max_width' => '',
			'max_height' => '',
			'max_size' => '',
			'mime_types' => '',
			'preview_size' => 'medium',
		),
		array(
			'key' => 'field_6666eb0badb67',
			'label' => 'Image_4',
			'name' => 'image_4',
			'aria-label' => '',
			'type' => 'image',
			'instructions' => '',
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'return_format' => 'array',
			'library' => 'all',
			'min_width' => '',
			'min_height' => '',
			'min_size' => '',
			'max_width' => '',
			'max_height' => '',
			'max_size' => '',
			'mime_types' => '',
			'preview_size' => 'medium',
		),
		array(
			'key' => 'field_6666eb27adb68',
			'label' => 'Image_5',
			'name' => 'image_5',
			'aria-label' => '',
			'type' => 'image',
			'instructions' => '',
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'return_format' => 'array',
			'library' => 'all',
			'min_width' => '',
			'min_height' => '',
			'min_size' => '',
			'max_width' => '',
			'max_height' => '',
			'max_size' => '',
			'mime_types' => '',
			'preview_size' => 'medium',
		),
		array(
			'key' => 'field_6666c0475d49d',
			'label' => 'Year',
			'name' => 'year',
			'aria-label' => '',
			'type' => 'text',
			'instructions' => '',
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'default_value' => '',
			'maxlength' => '',
			'placeholder' => '',
			'prepend' => '',
			'append' => '',
		),
		array(
			'key' => 'field_6666c0b03a4c6',
			'label' => 'Photographer',
			'name' => 'Photographer',
			'aria-label' => '',
			'type' => 'text',
			'instructions' => '',
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'default_value' => '',
			'maxlength' => '',
			'placeholder' => '',
			'prepend' => '',
			'append' => '',
		),
		array(
			'key' => 'field_6666c35f599ac',
			'label' => 'Ontwerper',
			'name' => 'Ontwerper',
			'aria-label' => '',
			'type' => 'text',
			'instructions' => '',
			'required' => 0,
			'conditional_logic' => 0,
			'wrapper' => array(
				'width' => '',
				'class' => '',
				'id' => '',
			),
			'default_value' => '',
			'maxlength' => '',
			'placeholder' => '',
			'prepend' => '',
			'append' => '',
		),
	),
	'location' => array(
		array(
			array(
				'param' => 'post_type',
				'operator' => '==',
				'value' => 'portfolio',
			),
		),
	),
	'menu_order' => 0,
	'position' => 'normal',
	'style' => 'default',
	'label_placement' => 'top',
	'instruction_placement' => 'label',
	'hide_on_screen' => '',
	'active' => true,
	'description' => '',
	'show_in_rest' => 0,
) );
} );

